...
- Ramas: feature/_, fix/_, chore/_, docs/_, release/\*.
...
export const inventoryService = {
  ...
  subscribeToInventory(callback) {
    const q = query(collection(db, COLLECTIONS.INVENTORY), orderBy('updatedAt', 'desc'));
...
const renderWithProvider = (component) => {
  return render(<CombustiblesProvider>{component}</CombustiblesProvider>);
};
...
describe('useCombustiblesCRUD', () => {
  ...
  it('should handle error states correctly', async () => {
    const mockDeleteMovement = vi.fn(() => Promise.reject(new Error('Firebase connection failed')));
...
import { connectFirestoreEmulator, doc, setDoc, collection, getDocs } from 'firebase/firestore';
...
test.describe('Fuel Movement Flow', () => {
  ...
  test('should complete fuel exit movement successfully', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="movements-table"]')).toContainText(
  ...
  test('should prevent invalid fuel quantities', async ({ page }) => {
    ...
    await expect(page.locator('.error-message')).toContainText('cantidad excede el límite');
  ...
  const stats = useMemo(() => {
    return {
      ...
      maintenanceNeeded: vehicles.filter(
        (v) => v.nextMaintenance && new Date(v.nextMaintenance) < new Date()
      ).length,
      fuelConsumption: vehicles.reduce((acc, v) => acc + (v.monthlyFuelConsumption || 0), 0),
      avgHorometer: vehicles.reduce((acc, v) => acc + (v.horometer || 0), 0) / vehicles.length,
    };
  }, [vehicles]);
    ...
    rollupOptions: {
      output: {
        manualChunks: {
          ...
          'ui-vendor': ['@heroicons/react', 'chart.js', 'react-chartjs-2', 'date-fns'],
...
export const AppRoutes = () => {
  return (
    <Routes>
      <Route path="/inventory" element={withLazyLoading(InventoryMain)} />
      <Route path="/movements" element={withLazyLoading(MovementsMain)} />
      <Route path="/vehicles" element={withLazyLoading(VehiclesMain)} />
      <Route path="/maintenance" element={withLazyLoading(MaintenanceMain)} />
      <Route path="/reports" element={withLazyLoading(ReportsMain)} />
    </Routes>
  );
};
```
...
export const usePerformanceMonitor = (componentName) => {
  ...
  return {
    ...
    measureAsync: (name, fn) => performanceMonitor.measureOperation(`${componentName}.${name}`, fn),
  };
};
```
  ...
  return (
    <div className={`error-fallback ${level}-level`}>
      <div className="error-content">
        <h2>{isAppLevel ? '¡Ops! Algo salió mal' : 'Error en este componente'}</h2>
...
export const useServiceCall = () => {
  ...
  const callService = useCallback(
    async (serviceFunction, args = [], options = {}) => {
      const {
        showSuccessMessage,
...
export const useOnlineOperation = () => {
  ...
  const executeIfOnline = useCallback(
    (operation, fallbackMessage) => {
      if (!isOnline) {
        showWarning(fallbackMessage || 'Esta acción requiere conexión a internet');
...
class Logger {
  ...
  exportLogs(filter = {}) {
    ...
    if (filter.category) {
      filteredLogs = filteredLogs.filter((log) => log.data.category === filter.category);
    }
  ...
  enableDebugMode() {
    ...
    window.debugComponents = {
      ...
      trackReRenders: (componentName) => {
        ...
        window.rerenderCounts[componentName] = (window.rerenderCounts[componentName] || 0) + 1;
