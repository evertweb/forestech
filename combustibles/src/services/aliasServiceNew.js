...
class AliasService extends CRUDService {
  ...
  validateData(data) {
    ...
    if (!data.type || !Object.values(ALIAS_TYPES).includes(data.type)) {
      errors.push(`Tipo de alias inválido. Debe ser uno de: ${Object.values(ALIAS_TYPES).join(', ')}`);
    }
  ...
  async saveAliases(aliasType, mappings, user) {
    try {
      this.log('saveAliases', { aliasType, mappingsCount: Object.keys(mappings).length, user: user?.email });
  ...
  async getSuggestedMappings(aliasType, inputNames, existingData, options = {}) {
    try {
      this.log('getSuggestedMappings', { aliasType, inputCount: inputNames.length, existingCount: existingData.length });
...
export const saveAliases = (aliasType, mappings, user) => service.saveAliases(aliasType, mappings, user);
export const resolveAlias = (aliasType, inputName, caseSensitive) => service.resolveAlias(aliasType, inputName, caseSensitive);
export const getSuggestedMappings = (aliasType, inputNames, existingData, options) => service.getSuggestedMappings(aliasType, inputNames, existingData, options);
export const recordAliasUsage = (aliasType, aliasName) => service.recordAliasUsage(aliasType, aliasName);
export const cleanupUnusedAliases = (aliasType, daysUnused) => service.cleanupUnusedAliases(aliasType, daysUnused);
export const exportAliases = (aliasType) => service.exportAliases(aliasType);
export const importAliases = (aliasType, importData, user) => service.importAliases(aliasType, importData, user);
export const createAliasDocument = (type, mappings, metadata) => service.createAliasDocument(type, mappings, metadata);
