...
class BackgroundImageService extends CRUDService {
    ...
    return new Promise((resolve) => {
      ...
      img.onload = () => {
        ...
        if (img.width < config.minDimensions.width || img.height < config.minDimensions.height) {
          errors.push(`Dimensiones muy pequeñas. Mínimo: ${config.minDimensions.width}x${config.minDimensions.height}`);
        }
        
        if (img.width > config.maxDimensions.width || img.height > config.maxDimensions.height) {
          errors.push(`Dimensiones muy grandes. Máximo: ${config.maxDimensions.width}x${config.maxDimensions.height}`);
        }
  ...
  async getUsageStats(days = 7) {
    try {
      ...
      stats.successRate = stats.totalAccesses > 0 
        ? Math.round((stats.successfulAccesses / stats.totalAccesses) * 100) 
        : 0;

      stats.fallbackRate = stats.totalAccesses > 0 
...
export const uploadBackgroundImage = (file, userId) => backgroundImageService.uploadBackgroundImage(file, userId);
export const deleteBackgroundImage = (userId) => backgroundImageService.deleteBackgroundImage(userId);
...
export const validateImageDimensions = (file) => backgroundImageService.validateImageDimensions(file);
