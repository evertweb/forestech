...
class DataResetService extends CRUDService {
  ...
  processData(data, isUpdate = false) {
    ...
    baseProcessed.userAgent = typeof window !== 'undefined' ? 
      window.navigator.userAgent.substring(0, 200) : 'Unknown';
  ...
  async validateResetPermission(userId, confirmationPhrase) {
    try {
      ...
      if (confirmationPhrase !== SECURITY_CONFIG.confirmationPhrase) {
        errors.push(`Frase de confirmación incorrecta. Debe ser: "${SECURITY_CONFIG.confirmationPhrase}"`);
      }
      ...
      if (recentAttempts.length >= SECURITY_CONFIG.maxResetAttempts) {
        ...
        if (timeSinceLastAttempt < SECURITY_CONFIG.cooldownPeriod) {
          const remainingTime = Math.ceil((SECURITY_CONFIG.cooldownPeriod - timeSinceLastAttempt) / 1000 / 60 / 60);
          errors.push(`Demasiados intentos. Espere ${remainingTime} horas antes de intentar nuevamente`);
        }
      }

      // Verificar que no hay reseteos activos
      const activeResets = Array.from(this.activeResets.values())
        .filter(reset => reset.status === RESET_STATUS.IN_PROGRESS);
    ...
    try {
      ...
      const resetContext = {
        ...
        progress: {
          ...
          totalSteps: DELETION_ORDER.length + (options.createBackup ? 1 : 0) + (options.restoreBasicData ? 1 : 0),
          completedSteps: 0,
          collections: {}
        },
        errors: [],
...
export const startDataReset = (userId, confirmationPhrase, options) => dataResetService.startDataReset(userId, confirmationPhrase, options);
export const onResetProgress = (resetId, callback) => dataResetService.onResetProgress(resetId, callback);
