...
class FileParsingService extends CRUDService {
  ...
  validateFile(file) {
    ...
    if (file.size === 0) {
      ...
      errors.push(
        `El archivo es demasiado grande: ${(file.size / 1024 / 1024).toFixed(2)}MB. Máximo: ${this.maxFileSize / 1024 / 1024}MB`
      );
    }
  ...
  async parseExcelFile(file, options = {}) {
    try {
      ...
      if (!workbook.Sheets[targetSheet]) {
        throw new Error(
          `Sheet '${targetSheet}' no encontrado. Sheets disponibles: ${sheetNames.join(', ')}`
        );
      }
      ...
      this.log(
        `Excel parseado exitosamente: ${result.metadata.rowCount} filas, ${result.metadata.columnCount} columnas`
      );
  ...
  async parseCSVFile(file, options = {}) {
    try {
      ...
      const lines = text
        .split('\n')
        .filter((line) => (config.skipEmptyLines ? line.trim() !== '' : true));
      ...
      this.log(
        `CSV parseado exitosamente: ${result.metadata.rowCount} filas, ${result.metadata.columnCount} columnas`
      );
  ...
  processExcelData(jsonData) {
    ...
    for (let i = 0; i < jsonData.length; i++) {
      ...
      if (hasContent) {
        ...
        warnings.push(
          createParsingWarning('content', `Fila ${i + 2} omitida por estar vacía`, i + 2)
        );
      }
    }
  ...
  async getParsingStats(days = 7) {
    try {
      ...
      stats.successRate =
        stats.totalFiles > 0 ? Math.round((stats.successfulFiles / stats.totalFiles) * 100) : 0;
