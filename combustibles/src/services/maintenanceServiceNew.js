...
class MaintenanceService extends CRUDService {
  ...
  subscribeToMaintenance(callback, filters = {}) {
    ...
    return this.subscribeToChanges((maintenanceRecords, error) => {
      if (error) {
        callback([], error);
        return;
      }
      ...
      if (!vehicleSnapshot.empty) {
        ...
        if (vehicleData.hasHourMeter && vehicleData.type === 'tractor') {
          ...
          if (newHoursInt > currentHours) {
            ...
            this.logInfo('updateVehicleHourMeter', `Horómetro actualizado: ${vehicleId} - ${currentHours}h → ${newHoursInt}h`);
          }
        }
      }
    } catch (error) {
...
export const getMaintenanceByVehicle = (vehicleId) => maintenanceService.getMaintenanceByVehicle(vehicleId);
...
export const subscribeToMaintenance = (callback, filters) => maintenanceService.subscribeToMaintenance(callback, filters);
export const getVehiclesForMaintenance = () => maintenanceService.getVehiclesForMaintenance();
export const calculateNextOilChange = (currentHours) => maintenanceService.calculateNextOilChange(currentHours);
export const calculateNextBatteryChange = (lastChangeDate) => maintenanceService.calculateNextBatteryChange(lastChangeDate);
