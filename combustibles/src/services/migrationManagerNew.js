...
class MigrationManager extends CRUDService {
  ...
  async configureColumnMapping(sessionId, columnMapping) {
    try {
      ...
      if (context.step !== MIGRATION_STEPS.COLUMN_MAPPING) {
        throw new Error(`Paso incorrecto. Esperado: ${MIGRATION_STEPS.COLUMN_MAPPING}, Actual: ${context.step}`);
      }
  ...
  async configureValueMapping(sessionId, valueMapping) {
    try {
      ...
      if (context.step !== MIGRATION_STEPS.VALUE_MAPPING) {
        throw new Error(`Paso incorrecto. Esperado: ${MIGRATION_STEPS.VALUE_MAPPING}, Actual: ${context.step}`);
      }
  ...
  async validateMigrationData(sessionId) {
    try {
      ...
      const mappedData = this.applyMappings(context.fileData, context.columnMapping, context.valueMapping);
  ...
  async executeMigration(sessionId, options = {}) {
    try {
      ...
      if (context.step !== MIGRATION_STEPS.EXECUTION) {
        throw new Error(`Paso incorrecto. Esperado: ${MIGRATION_STEPS.EXECUTION}, Actual: ${context.step}`);
      }
      ...
      this.log(`Migración ${executionResult.success ? 'completada' : 'falló'} para sesión: ${sessionId}`, {
  ...
  async getMigrationStats(days = 7) {
    try {
      ...
      const stats = {
        ...
        completedSessions: recentSessions.filter(s => s.status === MIGRATION_SESSION_STATUS.COMPLETED).length,
        failedSessions: recentSessions.filter(s => s.status === MIGRATION_SESSION_STATUS.FAILED).length,
        cancelledSessions: recentSessions.filter(s => s.status === MIGRATION_SESSION_STATUS.CANCELLED).length,
      ...
      const completedSessions = recentSessions.filter(s => 
        s.status === MIGRATION_SESSION_STATUS.COMPLETED && s.metadata
      );

      if (completedSessions.length > 0) {
        stats.totalRowsProcessed = completedSessions.reduce(
          (sum, s) => sum + (s.metadata.processedRows || 0), 0
        );
...
export const createMigrationSession = (userId, options) => migrationManager.createMigrationSession(userId, options);
export const processFile = (sessionId, file, options) => migrationManager.processFile(sessionId, file, options);
export const configureColumnMapping = (sessionId, mapping) => migrationManager.configureColumnMapping(sessionId, mapping);
export const configureValueMapping = (sessionId, mapping) => migrationManager.configureValueMapping(sessionId, mapping);
export const validateMigrationData = (sessionId) => migrationManager.validateMigrationData(sessionId);
export const executeMigration = (sessionId, options) => migrationManager.executeMigration(sessionId, options);
export const onSessionProgress = (sessionId, callback) => migrationManager.onSessionProgress(sessionId, callback);
