...
class ProductCategoriesService extends CRUDService {
  ...
  async searchProductCategories(searchTerm) {
    try {
      ...
      const filtered = allCategories.data.filter(
        (category) =>
    ...
    try {
      ...
      const statsPromises = allCategories.data.map(async (category) => {
        try {
          ...
          const activeProducts = snapshot.docs.filter(
...
export const updateProductCategory = (categoryId, data, user) =>
  service.updateProductCategory(categoryId, data, user);
export const deleteProductCategory = (categoryId, user) =>
  service.deleteProductCategory(categoryId, user);
