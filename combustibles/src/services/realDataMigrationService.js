...
const REAL_DATA = {
  ...
  movements: [
    ...
    { codigo: 'G', fecha: '11/24/2023', articulo: 'GASOLINA', usuario: 'Vivero', cantidad: '1' },
    {
      ...
      fecha: '11/25/2023',
      articulo: 'GASOLINA',
      usuario: 'MOTOBOMBA CAMPAMENTO ILUSION',
      cantidad: '1',
    },
    {
      codigo: 'G',
...
class RealDataMigrationService {
  ...
  async executeMigration(onProgress) {
    try {
      ...
      onProgress &&
        onProgress({ step: 'Migrando vehículos con horómetros reales...', progress: 10 });
      await this.migrateRealVehicles();

      // FASE 2: Migrar TODOS los 1,446+ movimientos reales
      onProgress &&
        onProgress({ step: 'Migrando 1,446+ movimientos históricos reales...', progress: 30 });
      await this.migrateAllRealMovements();

      // FASE 3: Aplicar mantenimientos con horómetros actualizados
      onProgress &&
        onProgress({
          step: 'Aplicando mantenimientos con horómetros hasta abril 2025...',
          progress: 70,
        });
  ...
  async migrateAllRealMovements() {
    ...
    for (let i = 0; i < allMovements.length; i += batchSize) {
      ...
      console.log(
        `📊 Progreso: ${Math.min(i + batchSize, allMovements.length)}/${allMovements.length} movimientos`
      );
    }
  }
  ...
  async createVehicleDocument(vehicleData) {
    try {
      ...
      console.log(
        `✅ Vehículo ${vehicleData.vehicleId} creado con horómetro ${vehicleData.currentHours}h`
      );
      return docRef.id;
    } catch (error) {
  ...
  async processMaintenanceRecord(maintenance) {
    console.log(
      `🔧 Procesando mantenimiento real: ${maintenance.maquina} - ${maintenance.horometro}h - ${maintenance.fecha}`
    );
  ...
  async processInventoryEntry(entry) {
    console.log(
      `📦 Procesando entrada real: ${entry.articulo} - ${entry.cantidad} - ${entry.fechas}`
    );
  ...
  async finalizeMigration() {
    try {
      ...
      console.log(
        '✅ Migración completa de datos reales completada exitosamente:',
        this.migrationId
      );
    } catch (error) {
