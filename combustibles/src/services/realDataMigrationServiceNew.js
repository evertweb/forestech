...
const REAL_DATA = {
  ...
  maintenance: [
    ...
    },
    {
      ...
      horometro: '4198',
      fecha: '3/20/2025',
      filtros: 'RE522868  FILTRO DE COMBUSTIBLE, FILTRO DE AIRE',
    },
    {
      año: '2025',
      maquina: 'TR3',
      ...
      horometro: '',
      fecha: '4/17/2025',
      filtros: 'RE50836 ACEITE, RE522868  COMBUSTIBLE, FILTRO DE AIRE',
    },
    {
      año: '2025',
      maquina: 'TR1',
...
class RealDataMigrationService extends CRUDService {
  ...
  async migrateRealProducts(productsData) {
    ...
    this.log(
      `Migración de productos reales completada: ${this.progress.products.processed}/${this.progress.products.total}`
    );
  }
  ...
  async migrateRealMovements(movementsData) {
    ...
    for (const movementData of movementsData) {
      try {
        ...
        if (batchCount >= BATCH_SIZE) {
          ...
          batchCount = 0;
          this.log(
            `Batch de movimientos ejecutado: ${this.progress.movements.processed}/${this.progress.movements.total}`
          );
        }
      } catch (error) {
        this.progress.movements.errors++;
    ...
    this.log(
      `Migración de movimientos reales completada: ${this.progress.movements.processed}/${this.progress.movements.total}`
    );
  }
  ...
  async migrateRealMaintenance(maintenanceData) {
    ...
    for (const maintenanceItem of maintenanceData) {
      try {
        ...
        this.log(
          `Mantenimiento real migrado: ${maintenanceItem.maquina} - ${maintenanceItem.fecha}`,
          {
            id: result.id,
          }
        );
    ...
    this.log(
      `Migración de mantenimiento real completada: ${this.progress.maintenance.processed}/${this.progress.maintenance.total}`
    );
  }
  ...
  async migrateRealEntries(entriesData) {
    ...
    this.log(
      `Migración de entradas reales completada: ${this.progress.entries.processed}/${this.progress.entries.total}`
    );
  }
  ...
  calculateSuccessRate() {
    const total =
      this.progress.products.total +
      this.progress.movements.total +
      this.progress.maintenance.total +
      this.progress.entries.total;
    const processed =
      this.progress.products.processed +
      this.progress.movements.processed +
      this.progress.maintenance.processed +
  ...
  async getRealMigrationStats() {
    try {
      ...
      const stats = {
        ...
        successfulMigrations: migrations.filter((m) => m.status === REAL_MIGRATION_STATUS.COMPLETED)
          .length,
      ...
      if (migrations.length > 0) {
        ...
        if (completedMigrations.length > 0) {
          stats.avgDuration = Math.round(
            completedMigrations.reduce((sum, m) => sum + (m.summary.duration || 0), 0) /
              completedMigrations.length
          );

          stats.totalRecordsProcessed = completedMigrations.reduce(
            (sum, m) => sum + (m.summary.totalProcessed || 0),
            0
          );
        }
      }
...
export const startRealDataMigration = (customData) =>
  realDataMigrationService.startRealDataMigration(customData);
