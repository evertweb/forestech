...
class VehicleCategoriesService extends CRUDService {
  ...
  async getCategoryStats() {
    try {
      ...
      const stats = categoriesResult.data
        .map((category) => {
          ...
          return {
            ...
            totalFuelCapacity: vehiclesInCategory.reduce(
              (sum, v) => sum + (v.fuelCapacity || 0),
              0
            ),
            avgEnginepower:
...
export const createCategory = (categoryData, user) =>
  service.createVehicleCategory(categoryData, user);
...
export const updateCategory = (categoryId, categoryData, user) =>
  service.updateVehicleCategory(categoryId, categoryData, user);
