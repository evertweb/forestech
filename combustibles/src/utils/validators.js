...
export const validators = {
  ...
  range: (min, max, message) => (value) => {
    ...
    return num < min || num > max ? message || `Debe estar entre ${min} y ${max}` : null;
  },
  ...
  stockCapacityCheck: (
    currentStock,
    maxCapacity,
    message = 'Stock no puede exceder la capacidad'
  ) => {
  ...
  thresholdCapacityCheck: (
    threshold,
    maxCapacity,
    message = 'Umbral no puede exceder la capacidad'
  ) => {
